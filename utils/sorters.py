...
def date_sort(image_paths: List[Path]):
    ...
    if image_dates:
        return [image_data["path"] for image_data in sorted(image_dates, key=lambda x: x["datetime_original"])]
    else:
        print(" * Couldn't sort images due to lack of *datetime_original* exif attribute.")
        return image_paths
